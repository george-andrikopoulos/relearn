...
- [ ] **`Destination` is still a string that encodes a list.** Three graduated rules pack two controls into one `to` with `" + "` and parenthetical scopes (`"hook:gate-verdict-intact (pipeline half) + hook:multiline-pattern-eol (edit half)"`). A typed `Controls` — a non-empty list of `Control { kind, name, covers }` — is the literal reading of "single-destination" and was **deliberately deferred** on 2026-09-16 as separable: it migrates the on-disk format of all eight graduated rules and touches the federation surface, where this change touched neither. The harm is fixed; the smell is not. Nothing enforces the `" + "` convention and nothing ever will while it is prose inside a string.
- [ ] **Nothing stops the next `matches!` on a domain enum.** The 2026-09-16 sweep converted nine existing decisions into exhaustive policy methods, and the compiler now blocks a new variant at each — but it says nothing about a *new call site* written tomorrow as `matches!(rule.status(), Status::Active)`. The obvious gate is unbuildable as stated: a grep ban on `matches!` over `Status | Authority | Origin | Home` also bans the legitimate identity test and every `assert!(matches!(..))` in the test suite, and a gate that must be muted for correct code is a muted gate (`[R:xplat-fixtures]`, the always-red half). A narrower one — ban `matches!` on those four enums in non-test lines outside their own modules — is buildable and would have caught all nine. Not built: the sweep took priority and the check needs its own argument about false positives. Until then this is a review question, which is the weakest layer.
