...
- [ ] **Reliance outranks recurrence, and the one case where they disagree is George's to confirm.** The rank puts `Status::instruction_reliance` above recurrence count, so a never-fired `Active` rule beats a recurred `Partial` one. The live instance is `[R:guarantee-needs-a-reader]` — `partial`, one recurrence, and now dropped from `global`'s description, where a recurrence-first rank would keep it. **The argument as implemented:** the rank answers "what does it cost for this title to be absent from this field", and that is a question about what *else* holds the rule; a partly-held rule has controls a wholly-unheld one does not. **The argument against:** recurrence is the number this whole framework treats as load-bearing — `lint` fails CI on an unheld one, the session-start hook leads with it — and putting a rule that has never fired above one that has inverts that everywhere else. Both are defensible; swapping the two keys is a one-line change in `emit::claude::description_order`. Nothing decided.
- [ ] **The rank decides which titles are dropped, not that titles are dropped — still open, and the cuts proved pruning is not the answer.** `global` drops fifteen of thirty; `domain-low-latency` will truncate again as it grows past about a dozen. Cutting two rules from the drafts took the drop from five to three and left the same rules invisible, because pruning changes the count and never the selection. What remains is the question the rank cannot answer: 1024 characters cannot hold both an inventory and a trigger, so the field has to stop being an inventory. The options from the load-path entry below stand, and (b) — derive trigger vocabulary from `error_class` rather than listing titles — is the one this makes most attractive, because it changes what the field is *for* instead of rationing what it lists.
...
- [ ] **Nothing checks that a documentation sample is a valid rule.** Found 2026-09-19: the front matter in `README.md` and `docs/worked-example.md` both omitted `origin`, and extracting either and running `relearn check` over it returned `missing field origin`. The project's public face showed a document the tool rejects, on both counts for months, while `tests/pack_counts.rs` caught a stale *line count* in the same session. The check is small — extract each `+++`-delimited block from `README.md` and `docs/*.md`, append a body if the sample omits one, and run it through `rule::parse_document` — and it would have failed on day one. The reason it is not already written is worth stating: a sample is prose to everyone reading it and a rule to nobody, so no existing gate had a reason to look at it. **Not built.** `[R:doc-currency]` `[R:wired-artifact]`
- [ ] **`title` and `error_class` travel on `contribute` unscanned — pre-existing, and George's call.** Both are authored free text and both reach the drive; only `published_incident`, `body` and now `source` are checked. Not introduced by the work above and deliberately not fixed there: widening the scan would newly **refuse contributions that pass today**, which is a policy change rather than a repair. The argument for widening is that nothing about a `title` prevents a product name in it; the argument against is that the two narrative fields are where a name actually lands, and a gate that starts refusing previously-legal input is how a gate gets muted. Recorded in FEATURES.md as `NOTHING YET — exposed` on the codified-source row.
- [ ] **`domain-low-latency` has no load path except a generated description — the highest-impact open item in the corpus.** `LoadSemantics::for_home` maps a domain to globs through `domain_globs`, which knows languages (`rust`, `java`, `c`, `cpp`, …). `low-latency` is not a language, returns no globs, and is therefore `OnRequest` — which `claude_rules` refuses to emit at all, confirmed by the absence of `.claude/rules/domain-low-latency.md` in the committed tree. So where `domain-rust` auto-attaches on any `.rs` read and `global` is always resident, **every low-latency rule reaches a Claude session only if the skill matcher fires on its `description`** — and that description is a generated inventory of titles with no trigger vocabulary and no "use when" clause. The 2026-09-06 work closed the description's *length* defect (6924 chars against a 1024 cap — uninstallable); it did not close the *trigger* defect, and the emitters-read-only-the-rule invariant means a hand-authored description is not available. This matters more than any rule body: a rule that never loads has no effect whatever, so this is the gate every cross-cutting domain sits behind. **Nothing decided.** The honest options are (a) let `Home::Domain` carry trigger vocabulary in the rule format, which is a format change and needs a decision about whose data it is — per-rule or per-home; (b) derive a trigger from `error_class` rather than `title`, which is free and stays inside the invariant, but error classes were removed from the description in 2026-09-06 for length; (c) accept `OnRequest` and invoke the skill by hand, which is honest and makes the domain opt-in; (d) extend `domain_globs` with non-language domains, rejected on sight — there is no file extension that means "this code has a latency budget", which is the same argument already recorded for `Org`.
