...
- [ ] **`[R:money-is-not-a-float]` ships with NO enforcing control, and this is the row that says so.** `status = active`, deliberately not `partial`: `Partial` must name what its controls do *not* cover, and here that is the entire class. The rule is layer 4, which is the layer this very failure defeated — `[R:verify-the-glyph-exists]` is the precedent, a rule read and then contradicted by the reader. `[R:features-ledger-names-its-artefact]` `[R:signal-needs-a-consequence]`
  - [ ] **Interim: a source scan.** Flag `f32`/`f64` declared as a struct field whose name matches a money vocabulary — `price`, `amount`, `qty`, `quantity`, `notional`, `value`, `balance`, `fee`, `commission`, `pnl`, `spread`, `cost`, `total`. Shaped like the Design-Architecture-Tool's font/window test rather than a grep: a scan that reads the *concept* — a numeric field in a monetary role — because the surface form varies and the concept does not. It must exclude its own definitions, or the vocabulary list will match the scanner and the check goes always-red `[R:detector-excludes-own-definitions]`.
  - [ ] **Attach it to the push, never to a command.** A gate someone must remember to invoke is the defect one level up, which is the entire content of `[R:signal-needs-a-consequence]` — whose own incident is a gate that existed and was not run. `.githooks/pre-push` is the precedent and the place.
