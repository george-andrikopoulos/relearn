...
- [ ] **The three ported rules now have TWO homes, exactly as the 2026-09-14 port did.** `~/.claude/CLAUDE.md` still states `[R:search-before-you-build]` and `[R:measure-the-claim-not-a-subset]` in full prose, `~/.claude/rules/ecc/common/agents.md` still states `[R:delegate-a-fan-out]`, and the compiled corpus now reaches that same machine through `~/.claude/skills/global/SKILL.md`. That is the P2 shape this tool exists to prevent, appearing in the tool's own supply chain. It is **deliberate for now**, on the 2026-09-14 precedent: `[R:attack-the-design-in-a-second-pass]` was ported and its CLAUDE.md prose was left standing. Not resolved here, because the fix edits the always-loaded global layer of a live machine and **nothing in this repository gates that file** — the same reason `docs/session-start-poke.md` is shipped and deliberately not installed. The honest options: reduce each section to a pointer at the compiled skill, or accept the boot layer as the index it already claims to be (CLAUDE.md describes its Patterns 1-9 that way in as many words). `relearn lint` can see neither option — `lint` takes `&Library<Validated>`, the Phase B exposure above. **George's call; nothing decided.**
