+++
tag = "R:a-measurement-matches-the-regime-it-reports"
title = "A measurement states whether it ran warm or cold, and matches production"
error_class = "Timing a path in a thermal and cache regime other than the one it runs in -- reporting a cold first execution as the steady state, or, run the other way, discarding warm-up on a path that is always cold in production and reporting a number no user has ever experienced"
home = { kind = "domain", name = "low-latency" }
applies_to = ["rust", "java"]
created = "2026-09-18"
origin = "codified"
source = "JMH and its samples (Aleksey Shipilev), whose existence is the standing claim that a hand-rolled microbenchmark on the JVM is wrong by default"
status = { kind = "active" }
incident = "Codification-dated, not single-incident: written 2026-09-18 from the same review. It is stated in both directions deliberately -- the forgotten warm-up is the well-known half and is caught in review; the unwanted warm-up is the half nobody calls a mistake, and it is how a path that is cold every time it matters comes to be reported by its warm number."
+++
