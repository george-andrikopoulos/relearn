+++
tag = "R:no-allocation-on-the-hot-path"
title = "A latency-bounded path does not enter the allocator"
error_class = "A path with a stated latency bound calling into the general-purpose allocator -- directly, or through a collection that grows, a boxed trait object, a formatted string or a per-message node -- so the path's worst case becomes the allocator's worst case: a lock, a page fault, an arena refill, or in a managed runtime a collection pause"
home = { kind = "domain", name = "low-latency" }
applies_to = ["rust", "java"]
created = "2026-09-18"
origin = "codified"
status = { kind = "active" }
incident = "Codification-dated, not single-incident: written 2026-09-18 from the same review. It is the rule that turns the other three from properties of an algorithm into properties of a system: a wait-free producer wrapped in a per-message allocation is not a wait-free producer, and the enclosing code is where that is lost, which is why an intrusive queue takes a node the caller already owns instead of allocating one."
+++
