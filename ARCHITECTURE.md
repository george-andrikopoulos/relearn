...
| `emit` | One submodule per target: `claude`, `cursor`, `copilot`, `agents`, `claude_md`. Pure functions `&Library<Validated> -> Vec<OutputFile>`. Shared scope/ordering/status helpers (`Scope`, `home_rank`, `HomeSlug`, `emittable`, `graduation_note`) | Read the filesystem, or read anything not carried by the rule |
