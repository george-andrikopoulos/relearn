---
name: domain-rust
description: "Engineering discipline for working in rust. Covers: Async is async all the way down; Take a borrow in the signature unless the function needs to own; Design by writing the types first, before any logic; Error enums carry the values that identify the failure; Every clone() carries its reason, or the design is wrong; Modules are the encapsulation boundary; pub is a deliberate export; Mark consequential return values #[must_use]; Newtype liberally: distinct concepts get distinct types; Parse, don't validate; Parse wide, then range-check; Struct fields are private; construction goes through a constructor; Seal a trait whose set of implementors is closed; Builders make a missing required field a compile error; Typestate for protocols: out-of-order calls should not compile; Verify an absent cost; reputation and a fast benchmark both lie; A test fixture must work on every OS the repository runs on; No anyhow in library return types; No unwrap() in production code"
---
