---
name: global
description: "Engineering discipline that applies to every project and language. Covers: Update the doc in the same change as the thing it describes; Put the costed fork; never resolve a trade of the user's resources silently; Verify through the production path; The project charter is written to the recreation standard; The decisions log records the why and what was rejected, append-only; The definition of done runs on every change, and skips are declared; Delegate a fan-out once the work has been enumerated; A detector excludes its own definitions from its scan; Every feature-ledger entry names the artefact that enforces it, or declares itself exposed; A project carries five standing documents, and resists a sixth; Make illegal states unrepresentable; Measure cost-per-completed-task; never choose by price tier; Match the scope before contradicting a recorded figure; Quoting an incident carries its names past the gate that was holding them; No sentinel values: absent states are enum variants; +15 more"
---
