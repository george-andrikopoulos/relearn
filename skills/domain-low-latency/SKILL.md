...
description: "Engineering discipline for working in low-latency. Covers: Attack the design in a separate pass, and build nothing in it; A measurement states whether it ran warm or cold, and matches production; An unbounded queue converts overload into latency instead of refusal; A structure is correct only under the concurrency regime its proof assumed; Memory that has never been touched is not yet memory; Answer a requirement at the layer it lives at; A latency-bounded path does not enter the allocator; A load generator that waits for the system deletes the system's worst latencies; Two hot values on one cache line contend without sharing anything; A contended path completes in a fixed number of steps, or its worst case is unbounded; A writer that can be stopped mid-publication makes the reader's latency the scheduler's; A syscall on a bounded path hands the deadline to the kernel; A sampling profiler reports where it was allowed to stop, not where the time went; +2 more"
---
...
<!-- relearn:generated v0.1.0 sha256=a2f2c67c09c46faf85a9e69e096b394c2f1522e9910fa1331e231105a20451af rules=R:a-measurement-matches-the-regime-it-reports,R:a-queue-without-a-bound-has-no-overload-behaviour,R:a-structure-keeps-the-regime-it-was-proved-under,R:allocated-is-not-resident,R:answer-the-requirement-at-its-layer,R:attack-the-design-in-a-second-pass,R:no-allocation-on-the-hot-path,R:no-coordinated-omission,R:no-false-sharing-on-a-hot-line,R:no-retry-loop-on-a-contended-path,R:no-stall-inside-a-publication-window,R:no-syscall-on-a-bounded-path,R:profiler-samples-where-it-can-stop,R:transient-state-is-not-a-terminal-state,R:verify-ordering-on-the-weakest-target -- DO NOT EDIT; regenerate with `relearn build` -->
